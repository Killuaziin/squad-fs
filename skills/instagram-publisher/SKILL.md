...
> **⚠️ Nota deste projeto (Forastabile, 2026-08):** a hospedagem de imagens descrita abaixo (imgBB) e a usada pelo `scripts/publish.js` (catbox.moe) estão DESATUALIZADAS em relação ao pipeline real. O caminho de publicação padrão do projeto é a **fila** (`queue/` + `publish-queue.mjs` no GitHub Actions), que serve as imagens via **raw.githubusercontent.com** (repositório público, sem host de terceiros — catbox/tmpfiles/0x0 quebraram em 2026-07). Use esta skill apenas para publicação direta pontual, ciente de que o upload via catbox pode falhar.
